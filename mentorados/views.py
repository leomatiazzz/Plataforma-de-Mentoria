...
def reunioes(request):
    if request.method == 'GET':
        ...
        messages.add_message(request, constants.SUCCESS, 'Horário de reunião agendado com sucesso.')
        return redirect('reunioes')
...
def auth(request):
    if request.method == 'GET':
        ...
        response.set_cookie('auth_token', token, max_age=3600)  # 1h
        return response
...
def escolher_dia(request):
    ...
    if request.method == 'GET':
        ...
        for i in disponibilidades:
            ...
            i.strftime('%B').encode('latin1').decode('utf-8').capitalize(),
            i.strftime('%A').encode('latin1').decode('utf-8').capitalize(),
    ))
        datas = list(OrderedDict.fromkeys(tuple(d) for d in datas))
...
def agendar_reuniao(request):
    ...
    if request.method == 'GET':
        ...
        # RESOLUÇÃO TO DO QUE FICOU PRA SER FEITO: VALIDAÇÃO PRA SABER HORÁRIO DO MENTORADO
        ...
        if horario.mentor != mentorado.user:
            messages.add_message(request, constants.ERROR, 'Você não pode agendar uma reunião com outro mentor.')
            return redirect('escolher_dia')
        reuniao = Reuniao(
            data_id=horario_id,
            mentorado=mentorado,
